...
class RpmBuild(steps.BuildStepMixin, unittest.TestCase):
    ...
    def test_success(self):
        ...
        self.expectCommands(
            ExpectShell(workdir='wkdir', command='rpmbuild --define "_topdir `pwd`" --define "_builddir `pwd`" --define "_rpmdir `pwd`" --define "_sourcedir `pwd`" --define "_specdir `pwd`" --define "_srcrpmdir `pwd`" --define "dist .el6" -ba foo.spec',
                        usePTY='slave-config')
